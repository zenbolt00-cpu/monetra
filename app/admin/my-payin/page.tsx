...
import { Loader2, Download, Filter, PiggyBank, Plus } from "lucide-react";
import { Button } from "@/components/ui/button";
import toast from "react-hot-toast";

export default function AdminMyPayinPage() {
  ...
  const fetchTransactions = useCallback(() => {
    fetch("/api/transactions?vendorId=admin&type=PAYIN")
      .then((res) => res.json())
      .then((data) => {
  ...
  const handleExport = () => {
    ...
    link.setAttribute("download", `admin_payin_records_${new Date().toISOString().slice(0, 10)}.csv`);
    ...
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-[#1d1d1f] flex items-center gap-3">
            <PiggyBank className="w-8 h-8 text-ios-green" /> Admin Pay-in
          </h1>
          <p className="text-[#86868b] mt-1">Managing administrator&apos;s direct incoming payment records.</p>
        </div>
      ...
      <div className="glass p-1 rounded-2xl border-black/5 flex flex-wrap items-center gap-2">
        <div className="flex items-center gap-3 px-4 py-2 border-r border-black/5">
          <Filter className="w-4 h-4 text-ios-green" />
          <span className="text-[10px] uppercase font-bold tracking-widest text-[#86868b]">Summary</span>
        </div>
        <div className="px-4 py-2">
          <p className="text-[10px] text-[#86868b] uppercase font-bold tracking-widest leading-tight">Total Self Pay-in</p>
