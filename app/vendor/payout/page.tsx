...
export default function VendorPayoutPage() {
  ...
  const handleExport = () => {
    ...
    link.setAttribute("download", `my_payout_records_${new Date().toISOString().slice(0, 10)}.csv`);
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-[#1d1d1f]">My Payout Records</h1>
          <p className="text-[#86868b] mt-1">Listing all payments sent and debit entries.</p>
        </div>
      ...
      <div className="glass p-4 rounded-2xl border-black/5 flex items-center gap-6">
        <div className="flex-1 relative">
          ...
          <input placeholder="Search records..." className="glass-input h-10 pl-10 border-transparent focus:border-black/5 w-full text-[#1d1d1f]" />
        </div>
        <div className="h-10 w-px bg-black/5" />
        <div>
          <p className="text-[10px] text-[#86868b] uppercase font-bold tracking-widest">Aggregate Payout</p>
